import type { ConversationTarget, LocalConversation } from "../../../../sdk";
...
export async function openConversation(
  ...
  if (conversation.kind === "group") {
    ...
    title = displayGroupName(
      app.client.getGroupInfos([groupId]).get(groupId),
      app.t("chat.group"),
    );
  } else {
    const friendUid = conv.friendUid || "0";
    app.client.getUserInfos([friendUid]);
    title = displayUserName(
      app.client.getUserInfos([friendUid]).get(friendUid),
      friendUid,
    );
  }
