...
import { createAuthView } from '../../src/uikit/app/views/auth';
import { initAfterAuth } from '../../src/uikit/app/main-app';

vi.mock('../../src/uikit/app/main-app', () => ({
  initAfterAuth: vi.fn(async (_app, options: { startSession?: () => Promise<void> }) => {
    await options.startSession?.();
  }),
}));

vi.mock('../../src/uikit/app/layout', () => ({
