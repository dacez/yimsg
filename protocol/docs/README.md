...
Yimsg 的主协议是 WebSocket 二进制帧。protobuf 是协议的单一事实源：所有 `Type`、request、response、错误码和 notification 都先在 `protocol/yimsg.proto` 中定义，再由 `go run ./tools/cmd/protocolgen` 调用标准 `protoc` 工具链生成 Go / TypeScript protobuf 类型。
...
```go
// server/internal/service/base_info.go
...
`go run ./tools/cmd/protocolgen` 以 `protocol/yimsg.proto` 为唯一事实源，先用 `protoc` 刷新 protobuf 类型，再由 `tools/protocolgen` 解析 proto 生成两端协议机械映射与协议文档。

| 生成目标 | 文件 |
|---|---|
| Go protobuf 类型与序列化代码 | `protocol/generated/go/pb/yimsg.pb.go` |
| TypeScript protobuf 类型与序列化代码 | `protocol/generated/typescript/yimsg.ts` |
| Go `ActionService` 接口（入方向） | `server/internal/ws/action_service_gen.go` |
| Go `DispatchActionFrame` 与 type 工厂 | `server/internal/ws/action_dispatch_gen.go` |
| Go 通知出方向 frame helper / `EncodeNotificationFrame` | `server/internal/ws/notification_frame_gen.go` |
| TS 出方向 action 无状态函数 | `packages/sdk/src/generated/actions.gen.ts` |
| TS 入方向 `NotificationHandler` 与 `dispatchNotificationFrame` | `packages/sdk/src/generated/notifications.gen.ts` |
| 协议接口表（自动生成，禁止手写） | `protocol/docs/协议接口表.md` |
| 协议中间清单 | `protocol/generated/protocol_manifest.json` |
...
1. 修改 `protocol/yimsg.proto`。
2. 确认 `node_modules` 中已有 `grpc-tools` / `ts-proto`，且本机可执行 `protoc-gen-go`。
...
当前已有跨语言 golden frame 回归：Go 测试 `server/internal/ws/golden_frame_test.go` 和 TypeScript 测试 `packages/sdk/tests/unit/sdk/protocol-golden-frame.test.ts` 各自硬编码同一组样例常量（`LoginRequest` 的 protobuf body，以及 big-endian / little-endian 两个完整 frame），共同校验 magic、codec、reserved、CRC、size、request_id、type 和 body 字节。
