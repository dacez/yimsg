...
run_tests() {
  ...
  local go_pkgs=()
  read_lines_into_array go_pkgs "go list ./... | grep -v -E '^yimsg/(server|cli|agent)/tests/e2e$'"
  run_step "go test (excluding server/tests/e2e, cli/tests/e2e, agent/tests/e2e)" go test -count=1 ${go_pkgs[@]+"${go_pkgs[@]}"}
  ...
  run_step "go test ./agent/tests/e2e/... -tls=false" go test -count=1 -v -timeout=3m ./agent/tests/e2e/... -tls=false -host="${SERVER_HOST}" -port="${SERVER_PORT}"
  (
    cd "${ROOT_DIR}"
