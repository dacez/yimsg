...
| 路径 | 职责 |
|---|---|
| `scripts/` | shell 脚本真实实现，包含全量测试、文档校验、覆盖率和质量门禁 |
...
根目录下的 `tools/*.sh` 是兼容入口，供文档、CI 和开发者继续使用稳定命令；新增脚本实现应放到 `tools/scripts/`。全量测试入口 `tools/scripts/run_all_tests.sh` 会准备前端依赖、安装 Playwright Chromium 依赖，并把固定版本的 `protoc-gen-go` 安装到 `$(go env GOPATH)/bin` 后再运行协议生成。
