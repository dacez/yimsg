...
> 主要对照：`server/internal/ws/connection.go`、`server/internal/service/`、`server/internal/dal/*_store.go`、`server/internal/appmsg/notification.go`、`packages/sdk/src/datagateway/sync-loop.ts`、`packages/sdk/src/datagateway/`。
> 最后复核：2026-08-25。
> 触发更新：消息、通讯录、屏蔽列表、免打扰或会话的 `get_*` / `sync_*` action、通知、游标、GC 水位、DataGateway 同步循环或持久存储本地同步表变化时同步更新。
> 入口关系：上级索引见 [`README.md`](../README.md)；服务端 action 和 HTTP 接口见 [`接口总览.md`](../../protocol/docs/接口总览.md)，SDK 对外 API 见 [`packages/sdk/docs/sdk接口说明.md`](../../packages/sdk/docs/sdk接口说明.md)，本文集中描述跨消息、通讯录、屏蔽列表、免打扰和会话的同步机制。
...
本文不替代字段级文档：完整 action、请求和响应字段仍以 [`接口总览.md`](../../protocol/docs/接口总览.md) 为准；表结构仍以 [`server/docs/db/schema字段对照.md`](../../server/docs/db/schema字段对照.md) 与各领域数据库文档为准；SDK 业务方 API 仍以 [`packages/sdk/docs/sdk接口说明.md`](../../packages/sdk/docs/sdk接口说明.md) 为准。
...
| `mutelist:updated` | 会话免打扰 | `mutelist:updated` |
| `conversations:clearunread` | 某会话未读被清除 | `conversations:clearunread`（SDK 不发起同步域拉取；UIKit 对窗口内目标调用 `getConversations({targets})` 定向刷新，不整列表重拉） |
| `conversations:delete` | 某会话被删除 | `conversations:delete`（SDK 不发起同步域拉取；UIKit 定向读取窗口内目标，返回空则移除并往上补齐） |
| `messages:delete` | 某消息被删除 | `messages:deleted`（SDK 删除本地副本；UIKit 就地删除消息并定向刷新会话预览） |
...
| 免打扰 | `get_mutelist` | `sync_mutelist` | `mutelist.seq` | `status=0xff` tombstone 表示关闭；持久本地删除该行 | `mutelist:updated` | `mutelist` |
...
`clear_unread` 只清零会话未读、不动 seq；会话行已被 GC（缺失）时**不重建**，后台会话表等下一条消息到来时自然重建追平。SDK DataGateway 收到 `conversations:clearunread` 后不发起同步域拉取：持久存储模式先清本地未读，再向 UI 发出携带会话 key 的事件。UIKit 只对仍在当前数据窗口内的 key 调用 `getConversations({targets})` 定向读取并更新红点，不整列表重拉；窗口外目标忽略。`conversations:delete` 同样由 SDK 先删本地副本并发出 key，UIKit 对窗口内目标定向读取，返回空时移除并往上补齐。`messages:delete` 由 SDK 删除本地消息并发出 `messages:deleted`，UIKit 就地删除消息并定向刷新对应会话预览。由于这些通知不启动对应同步域拉取，未读清除和删除不依赖增量同步传播。
...
展示排序（`rank, sort_key, child_type, child_id` 索引）与同步游标（`seq` 索引）正交。详见 [`server/docs/组织架构方案.md`](../../server/docs/组织架构方案.md)。

## 4. 服务端实现约定

### 4.1 WebSocket action

所有 action 都从已认证连接读取 `uid`，客户端不传当前用户 ID。展示通道 `get_*` 用 `PageQuery{cursor, direction, limit, around}`，响应带 `PageInfo`；同步通道 `sync_*` 用 `last_seq` / `limit` / `rebuild`，响应统一带 `has_more` / `cursor_seq`，下一页用上一页的 `cursor_seq` 作为 `last_seq` 继续。响应字段以 [`接口总览.md`](../../protocol/docs/接口总览.md) 的 action 表为准。
...
- 会话分页直接读服务端，`messages:received` 触发 UI 重读当前页；`conversations:clearunread` / `conversations:delete` 不启动 SDK 同步域拉取，UIKit 只对仍在窗口内的会话做 `getConversations({targets})` 定向读取，不整列表重读。
...
- 表级游标保存在 `meta`，包括 `msg_seq`、`contact_seq`、`conversation_seq`、`blocklist_seq`、`mutelist_seq`；组织按 org 一游标，键为 `org_seq:<orgId>`。游标读取不在 JS 堆维护镜像缓存，每次推进 / 使用前都直接查询 `meta` 表当前值，与组织游标 `orgCursor`/`saveOrgCursor` 同一模式。
...
完整测试分层见 [`测试方案.md`](../development/测试方案.md)。只改本文档和入口链接时可只运行文档一致性校验；修改实现时仍以 [`../tools/run_all_tests.sh`](../../tools/run_all_tests.sh) 为最终校验。

## 10. 维护边界

- 本文维护跨领域同步机制和共同规则。
- [`接口总览.md`](../../protocol/docs/接口总览.md) 维护 action、请求 / 响应字段和 SDK 映射。
- [`server/docs/db/通讯录数据库设计.md`](../../server/docs/db/通讯录数据库设计.md)、[`server/docs/db/消息数据库设计.md`](../../server/docs/db/消息数据库设计.md)、[`server/关系与会话偏好方案.md`](../../server/docs/关系与会话偏好方案.md) 维护领域特有字段、排序、业务行为和 GC 细节。
- [`packages/sdk/docs/sdk设计方案.md`](../../packages/sdk/docs/sdk设计方案.md) 维护 SDK 内部组件边界、生命周期、只读契约和内存估算。
- [`packages/sdk/docs/sdk接口说明.md`](../../packages/sdk/docs/sdk接口说明.md) 维护业务方可见 API 和调用约束。
