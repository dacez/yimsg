...
> 主要对照：`server/internal/ws/connection.go`、`server/internal/service/`、`server/internal/dal/*_store.go`、`server/internal/appmsg/notification.go`、`packages/sdk/src/datagateway/sync-loop.ts`、`packages/sdk/src/datagateway/`、`packages/sdk/src/worker/sqlite.worker.ts`。
> 最后复核：2026-07-29。
