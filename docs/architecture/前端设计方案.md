...
> 主要对照：`apps/web/src/main.ts`、`apps/web/src/home-dashboard-main.ts`、`packages/uikit/src/`、`packages/sdk/src/worker/`。
> 最后复核：2026-07-29。
...
persistent 模式下，设置页提供「清除数据」按钮：以 `startSession({ storage: 'persistent', resetLocalData: 'current-user', instanceId })` 重新初始化当前实例，删除本地 SQLite 副本后从服务端重新全量追平；不再是一个独立的启动模式。
