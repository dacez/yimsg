...
> 主要对照：`apps/web/src/main.ts`、`apps/web/src/home-dashboard-main.ts`、`packages/uikit/src/`、`packages/sdk/src/datagateway/`。
> 最后复核：2026-08-25。
> 触发更新：前端运行形态、启动装配、本地存储、SDK 内存边界或 SDK/UIKit 分层变化时同步更新。
> 入口关系：上级索引见 [`README.md`](前端文档索引.md)；通用同步机制见 [`../同步机制方案.md`](同步机制方案.md)，本文是前端运行形态、分层、存储与启动装配的总览入口。
> 本文只描述当前实现的稳定结构、边界和阅读路径，不再维护逐字段、逐流程的伪规格说明。更细的 UI、SDK、UIKit 接口细节分别见 uikit/UI设计方案.md、sdk/sdk设计方案.md、uikit/UIKit方案.md。
...
如果你要看具体 DOM 结构、交互规则和视图模块，请读 uikit/UI设计方案.md；如果你要看 SDK 能力、DataGateway、缓存、通知流和内存边界，请读 sdk/sdk设计方案.md；如果你要接入宿主页面，请读 uikit/UIKit方案.md。
...
| 运行形态 | 入口 | 宿主 | 说明 |
|---|---|---|---|
| 单实例完整应用 | apps/web/src/main.ts -> mountApp() | Light DOM | 项目自身默认入口，继续承载完整 Web / Web 应用 |
| 九宫格控制台 | apps/web/src/home-dashboard-main.ts -> mountHomeDashboard() | Light DOM 控制台 + 9 个 Shadow DOM UIKit 实例 | 独立页面直接渲染控制台外壳，每格提供 host 并调用 mount() 创建独立 Shadow DOM 聊天实例，同时提供加载 / 卸载 / 删数据 / 调尺寸控制 |
...
- 都会创建 AppInstance。
- 都会调用 startApp(app)。
- 都复用 packages/uikit/src/app/views/ 下的视图模块。
...
graph TB
  ...
  Embed[嵌入入口\npackages/uikit/src/embed.ts]

  subgraph UIKit 装配层
    AppEntry[mountApp\npackages/uikit/src/app.ts]
    MainApp[startApp / initAfterAuth\npackages/uikit/src/app/main-app.ts]
    AppInstance[AppInstance / AppDomScope / AppStorageScope\npackages/uikit/src/app/app-instance.ts]
  ...
  subgraph SDK 层
    ...
    DS[InstantDataGateway / PersistentDataGateway]
    Transport[WsTransport]
  end

  subgraph 本地与外部
    Storage[storage-base / isolated storage]
    本地持久化能力[本地持久存储\n仅 持久存储]
...
| 层 | 主要职责 | 当前代码入口 |
|---|---|---|
| 浏览器入口 | 启动应用，不放业务逻辑 | apps/web/src/main.ts |
| UIKit 装配层 | 组装视图、DOM、存储、布局、语言、事件订阅 | packages/uikit/src/app.ts、packages/uikit/src/app/main-app.ts |
| UIKit 视图层 | 处理认证、聊天、通讯录、设置等页面交互 | packages/uikit/src/app/views/ |
| SDK 层 | 连接、认证、会话状态、缓存、DataGateway 编排 | packages/sdk/src/ |
| DataGateway 层 | 屏蔽 instant / persistent 数据来源差异 | packages/sdk/src/datagateway/ |
| 本地存储层 | token、mode、layout、lang、本地 持久存储数据 | packages/uikit/src/app/storage-base.ts、packages/sdk/src/datagateway/persistent.ts |

## 4. 启动与装配流程

### 4.1 主应用启动

主应用入口非常薄：apps/web/src/main.ts 只做一件事，就是调用 mountApp()。
...
- WebSocket 连接和重连。
- 登录、登出、token 认证。
- startSession 以及 instant / persistent DataGateway 选择。
- 会话分页读取、待处理请求计数 / 分页、显示信息缓存。
- 消息发送、撤回、转发、上传、群组和联系人相关调用。
- 把底层通知整理成 UI 可消费的事件。

SDK 层的长期硬原则是内存严格可控：任何新增缓存、运行态集合、同步队列、事件载荷或分页读取能力，都必须有明确上限、合并/淘汰策略或生命周期释放路径；不得让 JS 堆内存随历史消息量、联系人总量、会话总量或通知频率无界增长。具体边界和默认值以 [`sdk设计方案.md`](../../packages/sdk/docs/sdk设计方案.md) §10 为准。
...
| DataGateway 待处理请求能力 | 红点通过 `getContactCount(CONTACT_STATUS_PENDING_INCOMING)` 驱动，只统计待我处理的请求；Requests Tab 分别拉取 `PENDING_INCOMING`（带接受/拒绝按钮，双向翻页）与 `PENDING_OUTGOING`（仅展示"等待验证"，一次性拉取）；联系人分页排序以服务端 `sort_key` 结果为准，搜索用 `search_text` |
...
| 模式 | 说明 |
|---|---|
| instant | 轻量模式，不依赖本地持久化能力，本地数据不持久化 |
| persistent | 请求持久化本地会话数据；具体浏览器存储实现由 SDK DataGateway 决定 |

持久化会话的 `fileSystem` 当前只有 `local`（Node.js 本地 SQLite）。**浏览器端不提供 SQLite 持久化**：Web 端（含嵌入第三方站点的 UIKit）一律使用内存会话，请求 `persistent` 会安静降级为 `instant`；`DbApi` 抽象保留，供本地客户端使用。

本地客户端可通过 `startSession({ storage: 'persistent', resetLocalData: 'current-user', instanceId })` 重新初始化当前实例，删除本地 SQLite 副本后从服务端重新全量追平。Web 端没有本地副本，不提供该入口。

主应用会在登录前做模式选择；嵌入式 UIKit 可以通过 mount options 直接指定 mode。

### 7.2 会话级持久化项

UI 层统一通过 `storage-base.ts` 的 `StorageScope`（由每个 `AppInstance` 各自持有一份、不跨实例共享）管理以下键：
...
研发阶段不维护本地库迁移逻辑；`schema_version` 不匹配时直接重建本地表，随后由 SDK 按后端数据重新同步。
...
package.json 当前以“标准 build 同时产出主应用和 UIKit”为准：
...
| 命令 | 作用 |
|---|---|
| `./tools/run_unit_tests.sh` | Go 非 E2E 与 SDK / UIKit / Web 单元测试 |
| `./tools/run_integration_tests.sh` | SDK 真实服务端集成测试 |
| `./tools/run_e2e_tests.sh` | Server / CLI / Agent E2E 与 Web `chromium-e2e` |
| `./tools/run_component_tests.sh` | BoundedList `chromium-component` 功能测试 |
| `./tools/run_performance_tests.sh` | BoundedList `chromium-performance` 独立性能门禁 |
| `./tools/run_all_tests.sh` | unit → integration → E2E → component 仓库级正确性校验，不包含性能门禁 |

仓库当前的约定是：涉及代码行为变更时，最终以 `./tools/run_all_tests.sh` 作为正确性收口验证；涉及 BoundedList 性能路径时还必须独立执行 `./tools/run_performance_tests.sh`。
...
2. packages/uikit/src/app.ts
3. packages/uikit/src/app/main-app.ts
4. packages/uikit/src/app/app-instance.ts
5. packages/uikit/src/app/views/chat.ts 和 chat/*
6. packages/sdk/src/client.ts
7. packages/sdk/src/client-session-runtime.ts

如果你要看专项主题，再跳转：

1. UI 设计方案.md：页面结构、视图职责、主要交互。
2. uikit/UIKit方案.md：主应用与嵌入态如何共享一套 UIKit，以及宿主页面怎么挂载、有哪些 options 和回调。
3. sdk/sdk设计方案.md：SDK、缓存、DataGateway、通知流和内存边界。
