...
强刷会更早暴露"对象已经不存在或自己已经没有权限"。这类失败不能只弹一个 toast 就把陈旧界面留在屏幕上。当前用两条互补判据：
...
**判据二：通讯录入口是否还在。** 单靠判据一不够：组织删除后服务端只是**异步**清理各成员的通讯录组织行，在那之前 `get_tags` 对组织根仍然返回空列表而不是错误，界面既不报错也不收起。因此 `contacts:updated` 到达时还要按 `getContacts({ orgIds: [当前组织] })` 复核入口是否仍在，入口没了就收起已打开的组织详情（`packages/uikit/src/app/views/contacts.ts` 的 `closeOrgPanelIfEntryGone()`，由 `main-app.ts` 的 `contacts:updated` 处理器调用）。复核请求本身失败时不做判定，避免把网络抖动当成"组织已消失"。
...
| Web E2E | `apps/web/tests/e2e/org.spec.ts` | 组织改名后弹层标题 / 面包屑 / 通讯录条目即时追平；组织删除后通讯录入口消失并连带收起失效详情 |
