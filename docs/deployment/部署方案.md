...
`demo_kf_1~3` 按 [`../../agent/docs/agent方案.md`](../../agent/docs/agent方案.md) §2.3 的私有 + 共享两层知识库回答问题。knowledge base 内容随代码进版本库，内置在 `agent/tools/cmd/seed-resources`（`go:embed`），每次部署都会先清空 `/opt/yimsg/agent_data` 再执行这个工具重新写入，不再依赖人工 SSH 到服务器手工维护 `.md` 文件：
