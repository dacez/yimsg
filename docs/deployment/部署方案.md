...
`demo_kf_1~3` 按 [`../../agent/docs/agent方案.md`](../../agent/docs/agent方案.md) §2.3 的私有 + 共享两层知识库回答问题，demo 用的内容维护在服务器本地，不进版本库（和 `/opt/yimsg/deepseek_api_key`、`/opt/yimsg/data` 一样是运行时数据，不是源码）：
