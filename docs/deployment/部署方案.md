...
5. **DeepSeek API Key 同样是人工操作，不经 GitHub Actions**：跟证书是同一类"部署流程只创建占位、真实内容人工一次性写入"的密钥文件，见第 13.4/13.5 节。新增机器时，`tools/scripts/init_server_env.sh` 会在 `/opt/yimsg/deepseek_api_key` 不存在时创建一个空占位文件，之后需要人工 SSH 上去写入真实的 DeepSeek 官方 API Key，写好后长期有效，不需要在 GitHub Secrets 里配置，也不会被后续的自动部署覆盖。
...
不需要配置 `DEEPSEEK_API_KEY` 这个 secret：yimsg-agent 的 DeepSeek API Key 参考 TLS 证书的做法改成了文件路径（`/opt/yimsg/deepseek_api_key`，见第 13.2/13.4 节），由人工 SSH 一次性写入，不经过 GitHub Actions，也就不需要在这里配置。
...
`agent.toml`（含 demo 账号密码）权限 `640`（`yimsg:yimsg`），`/opt/yimsg/deepseek_api_key`（含 DeepSeek key）权限 `600`（`yimsg:yimsg`），跟 `/etc/ssl/certs/yimsg.key` 同等级别地只允许 `yimsg` 账号自己读取。这个文件为空或不存在时 `config.Resolve` 会拒绝启动（见 `agent/config/config.go`），`Restart=on-failure` 会让它按 `RestartSec=5` 反复重启，这是预期状态，不代表机器故障。
