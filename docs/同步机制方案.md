...
> 入口关系：上级索引见 [`README.md`](README.md)；服务端 action 和 HTTP 接口见 [`接口总览.md`](protocol/接口总览.md)，SDK 对外 API 见 [`frontend/sdk/sdk接口说明.md`](frontend/sdk/sdk接口说明.md)，本文集中描述跨消息、通讯录、屏蔽列表、免打扰和会话的同步机制。
...
本文不替代字段级文档：完整 action、请求和响应字段仍以 [`接口总览.md`](protocol/接口总览.md) 为准；表结构仍以 [`server/db/schema字段对照.md`](server/db/schema字段对照.md) 与各领域数据库文档为准；SDK 业务方 API 仍以 [`frontend/sdk/sdk接口说明.md`](frontend/sdk/sdk接口说明.md) 为准。
...
所有 action 都从已认证连接读取 `uid`，客户端不传当前用户 ID。展示通道 `get_*` 用 `PageQuery{cursor, direction, limit, around}`，响应带 `PageInfo`；同步通道 `sync_*` 用 `last_seq` / `limit` / `rebuild`，响应统一带 `has_more` / `cursor_seq`，下一页用上一页的 `cursor_seq` 作为 `last_seq` 继续。响应字段以 [`接口总览.md`](protocol/接口总览.md) 的 action 表为准。
...
- 本文维护跨领域同步机制和共同规则。
- [`接口总览.md`](protocol/接口总览.md) 维护 action、请求 / 响应字段和 SDK 映射。
...
- [`frontend/sdk/sdk设计方案.md`](frontend/sdk/sdk设计方案.md) 维护 SDK 内部组件边界、生命周期、只读契约和内存估算。
- [`frontend/sdk/sdk接口说明.md`](frontend/sdk/sdk接口说明.md) 维护业务方可见 API 和调用约束。
