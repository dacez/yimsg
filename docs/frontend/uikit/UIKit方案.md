...
| `instanceId` | 当前挂载实例唯一标识，用于运行时和持久存储本地库隔离；未显式传时取挂载容器的 `id`，否则固定为 `'default'`（见 `docs/frontend/sdk/sdk设计方案.md` persistent DB 命名） |
...
- SDK 内核：[`sdk设计方案.md`](../sdk/sdk设计方案.md)
- SDK 对外接口：[`sdk接口说明.md`](../sdk/sdk接口说明.md)
