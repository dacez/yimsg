...
| DataGateway 层 | 屏蔽 instant / persistent 数据来源差异 | frontend/src/sdk/datagateway/ |
| 本地存储层 | token、mode、layout、lang、本地 持久存储数据 | frontend/src/uikit/app/storage-base.ts、frontend/src/sdk/datagateway/persistent.ts |
...
SDK 层的长期硬原则是内存严格可控：任何新增缓存、运行态集合、同步队列、事件载荷或分页读取能力，都必须有明确上限、合并/淘汰策略或生命周期释放路径；不得让 JS 堆内存随历史消息量、联系人总量、会话总量或通知频率无界增长。具体边界和默认值以 [`sdk设计方案.md`](sdk/sdk设计方案.md) §10 为准。
