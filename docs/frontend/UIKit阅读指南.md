...
```css
.btn-primary { background: #2e7df6; color: #ffffff; }
```
...
`style.css:1` 第一行（`:root{--primary:#2e7df6; ...}`）定义了一堆 `--xxx` 变量，这叫 **CSS 自定义属性**，是本项目的「设计 token」。

- 定义：`--primary: #2e7df6;`（在 `:root` 里定义，`:root` 表示整篇文档的根，相当于全局作用域）。
...
| 三栏同时可见 | 右侧详情面板 `display:none` 直接隐藏（`style.css:274`） |
