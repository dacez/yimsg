...
| 模式支持 | `memory` / `persistent` 由挂载参数指定，持久存储可在设置页随时「清除数据」 | 登录前模式选择 + localStorage 记忆 |
...
| 存储模式 | 支持 | `memory` / `persistent`，具体持久化能力由 SDK 判断；`persistent` 下设置页可随时「清除数据」重新追平 |
...
| `instanceId` | 当前挂载实例唯一标识，用于运行时和持久存储本地库隔离；未显式传时取挂载容器的 `id`，否则固定为 `'default'`（见 `docs/frontend/sdk设计方案.md` persistent DB 命名） |
...
| `mode` | `memory` / `persistent`，默认 `memory` |
...
| 单元测试 | `frontend/tests/unit/uikit-mode.test.ts`、`startup-mode.test.ts` | mode 分支、布局决策 |
| 单元测试 | `frontend/tests/unit/uikit-navigation.test.ts` | `switchView` 在 `chat-only` / `contacts-only` 显示范围下强制落回对应视图 |
...
| 单元测试 | `frontend/tests/unit/uikit-settings-clear-data.test.ts` | 设置页「清除数据」按钮：仅 persistent 模式展示、确认弹窗、resetLocalData=current-user 重新初始化、失败与降级分支 |
...
| UI 测试 | `frontend/tests/ui/settings.spec.ts` | 「清除数据」按钮可见性与端到端清空重同步 |
| UI 测试 | `frontend/tests/ui/*.spec.ts` | 主应用持久存储全量能力 |

## 11. 已知边界

- 嵌入态请求 `persistent` 但浏览器持久化能力预检查不可用时，会自动降级为 `memory`；若本地持久化能力初始化阶段失败，则通过错误流程交给宿主处理。
