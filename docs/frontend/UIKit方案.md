...
| `viewMode` | `full` / `chat-only`，默认 `full`；`chat-only` 隐藏底部导航栏，只保留会话列表 + 聊天视图 |
...
`UIKitViewMode = 'full' | 'chat-only'`，默认 `full`。
...
`chat-only` 适合宿主只需要嵌入会话能力的场景（例如网站客服组件），不需要联系人管理或个人设置入口。联系人 / 设置视图仍会被创建但始终不可见，不影响其内部状态。
...
| 单元测试 | `frontend/tests/unit/uikit-navigation.test.ts` | `switchView` 在 `chat-only` 显示范围下强制落回 chat、不推送非法路由 |
...
- UIKit 目前提供完整聊天 UI 或 `viewMode: 'chat-only'` 隐藏底部导航栏两种形态，不提供只读迷你浮窗或只渲染单会话（不含消息面板）的轻量组件。
