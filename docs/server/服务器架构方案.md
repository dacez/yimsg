...
各字段语义、`codec` 位域、CRC-8 参数、整包上限和解码顺序的权威说明见 [`../protocol/README.md`](../protocol/README.md)，本文不重复维护；服务端实现入口是 `internal/protocol/frame.go`。
...
完整 WebSocket interface、HTTP 接口、请求 / 响应字段和 SDK ↔ 服务端映射统一维护在 [`../protocol/接口总览.md`](../protocol/接口总览.md)。本文不再重复列出逐 type 字段表，只保留服务端架构层需要理解的 dispatch 边界：

| 类别 | 当前实现 | 权威清单 |
|---|---|---|
| 认证前 type | `register`、`login`、`authenticate` | [`../protocol/接口总览.md` § 3 服务端接口详解](../protocol/接口总览.md#3-服务端接口详解) |
| 认证后 type | 用户、通讯录、消息、群组、会话偏好等核心 type | [`../protocol/接口总览.md` § 3 服务端接口详解](../protocol/接口总览.md#3-服务端接口详解) |
...
1. 新增或调整 type 时，先更新 `internal/protocol/yimsg.proto`，运行 `go run ./tools/cmd/protocolgen`，再更新 `internal/ws/connection.go` 和对应 service，同步 [`../protocol/接口总览.md`](../protocol/接口总览.md)；本文只在 dispatch 分类、认证边界或后处理模式变化时更新。
...
| 保持 Go 标准仓库布局 | 不把仓库拆成 `backend/`、`protocol/` 等顶层五段式；`cmd/`、`internal/`、`tests/`、`tools/` 沿用 Go 生态约定 | 强行迁移会影响 import path、脚本、文档链接和部署入口；协议或后端未来需要独立发布时再整体规划 |
