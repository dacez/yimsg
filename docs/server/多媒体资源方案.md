...
> 主要对照：`cmd/server/main.go`、`internal/service/upload.go`、`frontend/src/uikit/app/views/`。
> 最后复核：2026-05-30。
...
| 文件 | 变更类型 | 内容 |
|------|---------|------|
| `internal/config/config.go` | 修改 | 新增 `MediaConfig` 结构体，加入 `Config` |
| `internal/dal/types.go` | 当前实现 | 定义 `MsgImage = 1`、`MsgFile = 3`，并保留 `MsgMarkdown = 4` / `MsgExt = 5` |
| `cmd/server/main.go` | 修改 | 新增 `/api/upload` 路由 + `/media` 静态服务 + 启动时创建目录 |
| `internal/service/upload.go` | 新建 | 上传处理器（鉴权、校验、存储、响应） |
