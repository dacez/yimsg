...
| 目标 | 推荐阅读 |
|---|---|
| 理解服务端整体实现 | [`服务器架构方案.md`](服务器架构方案.md) → [`../接口总览.md`](../接口总览.md) → [`推送事件方案.md`](推送事件方案.md) |
| 修改 WebSocket action | [`../接口总览.md`](../接口总览.md) → [`服务器架构方案.md`](服务器架构方案.md) → 对应业务专题 |
| 修改表结构说明 | [`db/数据库设计总览.md`](db/数据库设计总览.md) → [`db/schema字段对照.md`](db/schema字段对照.md) → 对应领域数据库文档 → [`../接口总览.md`](../接口总览.md) |
...
| [`../接口总览.md`](../接口总览.md) | WebSocket action、HTTP 上传 / 静态资源、SDK↔服务端接口映射、`client_config` | `internal/ws/connection.go`、`internal/protocol/`、`cmd/server/main.go` |
...
| [`db/schema字段对照.md`](db/schema字段对照.md) | 14 张表字段、主键、索引与用途对照 | `internal/dal/schema.go` |
...
1. `../接口总览.md` 负责列出完整服务端 action、HTTP 接口和 SDK 映射；专题文档只解释业务语义，不重复维护完整接口矩阵。
