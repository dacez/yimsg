...
func TestBlockPreventsFriendDMButNotGroupMessage(t *testing.T) {
	...
	if resp := addFriendService(s, "r2", uidB, uidA, ""); isOK(resp) || errMsg(resp) != "action not allowed while blocked" {
		t.Fatalf("add_friend while blocked = %+v", resp)
	}
	if resp := addFriendService(s, "r3", uidA, uidB, ""); isOK(resp) || errMsg(resp) != "action not allowed while blocked" {
		t.Fatalf("self add_friend while blocking = %+v", resp)
	}

	dmReq := &appmsg.Request{ToUID: uidB, MsgType: dal.MsgText, Content: "hi"}
	if result := sendMessageService(s, "r4", uidA, dmReq); isOK(result.Response) || errMsg(result.Response) != "recipient is not accepting private messages" {
		t.Fatalf("send dm while blocking = %+v", result.Response)
	}
	dmReq = &appmsg.Request{ToUID: uidA, MsgType: dal.MsgText, Content: "hi"}
	if result := sendMessageService(s, "r5", uidB, dmReq); isOK(result.Response) || errMsg(result.Response) != "recipient is not accepting private messages" {
