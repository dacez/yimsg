...
| 跨域访问 | 上传接口与 `/media/` 都经过跨域中间件：配置 `[server] allowed_origins` 后对命中的来源返回 CORS 头与 `Cross-Origin-Resource-Policy: cross-origin`，供第三方站点嵌入 UIKit 时使用。上传带 `Authorization` 头，跨域会先发 OPTIONS 预检 |
| 地址形态 | 上传响应与用户资料里的头像地址都是 `/media/{category}/{id}` **相对路径**（部署地址可变，存绝对地址会让数据随迁移失效）；跨域嵌入时由 SDK 的 `resolveMediaUrl()` 补上服务端基址 |
| 存储后端 | 本地文件系统 |
