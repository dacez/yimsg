...
> 主要对照：`server/cmd/yimsg-server/main.go`、`server/internal/ws/`、`server/internal/service/`、`server/internal/online/`、`server/internal/taskqueue/`。
> 最后复核：2026-07-19。
> 触发更新：服务端启动、dispatch、PostAction、在线态、异步任务队列或 GC 行为变化时同步更新。
> 入口关系：上级索引见 [`README.md`](README.md)；通用同步机制见 [`docs/architecture/同步机制方案.md`](../../docs/architecture/同步机制方案.md)，本文是服务端启动、模块边界、dispatch、PostAction、在线态、异步任务队列和 GC 的架构入口。
...
- 启动配置：`yimsg` 不带参数时使用内置默认值，并优先从当前目录、其次从可执行文件目录定位 `web/`、`website/` 和 `data/`，适合发行包解压即用；`--listen` 和 `--data-dir` 覆盖单机常用项，`--config` 或兼容的位置参数配置文件承载 TLS、分片和资源限制等高级项
...
- `dispatch` 返回 `(Response, postAction)`，postAction 是 Go int 常量（iota），描述需要执行的后处理：
  ...
  - 群消息扇出不再走 postAction：在 `sendMessage` 主流程中直接 `submitTask` 投递到异步任务队列（`server/internal/taskqueue`），由后台 worker 按分片分组并发写入，dispatch 不感知
...
```
server/cmd/yimsg-server ──→ server/internal/ws ──→ server/internal/service ──→ server/internal/dal ──→ server/internal/config
                                    │                    ↑
                                    └────────────────────┘
```
...
各字段语义、`codec` 位域、CRC-8 参数、整包上限和解码顺序的权威说明见 [`protocol/docs/README.md`](../../protocol/docs/README.md)，本文不重复维护；服务端实现入口是 `server/internal/ws/frame.go`。

Request / Response 结构规则：

- 每个 type 在 `protocol/yimsg.proto` 中定义独立 Request / Response。
...
完整 WebSocket interface、HTTP 接口、请求 / 响应字段和 SDK ↔ 服务端映射统一维护在 [`protocol/docs/接口总览.md`](../../protocol/docs/接口总览.md)。本文不再重复列出逐 type 字段表，只保留服务端架构层需要理解的 dispatch 边界：

| 类别 | 当前实现 | 权威清单 |
|---|---|---|
| 认证前 type | `register`、`login`、`authenticate` | [`protocol/docs/接口总览.md` § 3 服务端接口详解](../../protocol/docs/接口总览.md#3-服务端接口详解) |
| 认证后 type | 用户、通讯录、消息、群组、会话偏好等核心 type | [`protocol/docs/接口总览.md` § 3 服务端接口详解](../../protocol/docs/接口总览.md#3-服务端接口详解) |
...
1. 新增或调整 type 时，先更新 `protocol/yimsg.proto`，运行 `go run ./tools/cmd/protocolgen`，再更新 `server/internal/ws/connection.go` 和对应 service，同步 [`protocol/docs/接口总览.md`](../../protocol/docs/接口总览.md)；本文只在 dispatch 分类、认证边界或后处理模式变化时更新。
...
- **影响：** `rebuild=false && last_seq < gc_safe_seq` 的客户端会收到 `seq_too_old` 错误，恢复策略见 [`docs/architecture/同步机制方案.md`](../../docs/architecture/同步机制方案.md)

### 6.5 Blocklist GC

- **默认间隔：** 3600s（1 小时）；若运行时配置值 `<= 0`，内部兜底为 300s
- **操作：** 遍历所有 uid 分片，按 UID 游标扫描存在 `status=0xff` tombstone 的用户，物理删除已解除屏蔽记录并推进 `blocklist_version.gc_safe_seq`
- **影响：** 当前生效屏蔽列表不受影响；过旧屏蔽列表同步游标会收到 `seq_too_old` 错误，恢复策略见 [`docs/architecture/同步机制方案.md`](../../docs/architecture/同步机制方案.md)

### 6.6 Mute GC

- **默认间隔：** 3600s（1 小时）；若运行时配置值 `<= 0`，内部兜底为 300s
- **操作：** 遍历所有 uid 分片，按 UID 游标扫描存在 `status=0xff` tombstone 的用户，物理删除已关闭免打扰记录并推进 `mutelist_version.gc_safe_seq`
- **影响：** 当前静音会话不受影响；过旧免打扰同步游标会收到 `seq_too_old` 错误，恢复策略见 [`docs/architecture/同步机制方案.md`](../../docs/architecture/同步机制方案.md)
...
| 保持组件化 monorepo | 服务端位于 `server/`，协议位于 `protocol/`；Go module 名称保持 `yimsg` | 组件职责和许可证边界清晰，同时不拆分仓库、不引入 submodule |
...
- `server/cmd/yimsg-server/main.go`：启动流程、HTTP 路由、静态资源、上传入口或插件加载顺序。
- `server/internal/ws/connection.go`：action 分发、认证前拦截、PostAction 语义、在线注册与认证响应顺序。
- `server/internal/service/`：注册 / 登录、联系人、消息、群组、屏蔽列表、免打扰、撤回、GC 等业务流程。
- `protocol/` 与 `protocol/generated/go/pb/`：Request / Response / Notification 字段、错误语义或 `client_config`。
- `server/internal/dal/`、`server/internal/shard/`：分片路由、Store 边界、事务策略、GC 依赖的数据结构。
