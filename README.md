...
- 生产部署时官网默认挂载在服务端根路径 `/` 作为首页，聊天 App 挂载在子路径 `/chat/`：访客打开域名先看到产品介绍页，点击「打开应用」再进入聊天界面（挂载路径由 [`config.toml`](config.toml) 的 `[website]` / `[frontend]` 配置）
...
- **绝对安全**：所有数据集中存放在自己的机器里，不经过任何第三方云，完全自主可控，需要时可一键清空
- **极简部署**：一台机器即可上线，无需复杂运维；无论是否拥有域名或公网 IP，都有对应的接入方式
- **极致性能**：2 核 4G 单机即可支撑千人同时聊天，部署包不到 32MB，客户端内存占用可控（见下文「内存保证」），天然适合资源受限的嵌入式场景
