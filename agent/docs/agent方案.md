...
引擎用 DeepSeek 的 Chat Completions + Function Calling（`tools`）实现一个两阶段的 **Plan-and-Execute** 循环,而不是无约束的自由 ReAct(避免步骤数、工具调用数不可控)。三个自定义"工具"贯穿整个流程:

| 工具名 | 阶段 | 作用 |
|---|---|---|
| `list_md_files` | 决策阶段 + 每个步骤 | 列出 workspace 内某子目录下的 `.md` 文件 |
| `read_md_file` | 决策阶段 + 每个步骤 | 读取 workspace 内一个 `.md` 文件的内容 |
| `submit_plan` | 仅决策阶段 | 模型认为需要分步执行时,提交一个有序步骤描述列表,不直接调用即代表选择"直接回答" |

```mermaid
stateDiagram-v2
    [*] --> 决策阶段
    决策阶段 --> 决策阶段: 调用 list_md_files/read_md_file(有限次)
    决策阶段 --> 直接回答: 模型返回纯文本(无 tool_call)
    决策阶段 --> 步骤执行: 模型调用 submit_plan(steps)
    直接回答 --> [*]: FinalAnswer = 模型输出
    步骤执行 --> 步骤执行: 处理下一步(i < len(steps))
    步骤执行 --> 步骤执行: 每步内可调用 list_md_files/read_md_file(有限次)
...
2. 调用 DeepSeek,`tools = [list_md_files, read_md_file, submit_plan]`,`tool_choice = auto`。
3. 若模型返回 `list_md_files`/`read_md_file` 的 `tool_calls`:执行对应的 `fsread.Sandbox` 方法,把结果作为 `role=tool` 消息追加进对话,再次调用模型;这个子循环最多 `max_tool_calls_per_step` 次(默认 4),超过后强制要求模型基于已有信息直接结束(不再提供工具,只允许纯文本或 `submit_plan`)。
...
1. 组装一条 `user`/`developer` 消息:"当前执行第 `i+1`/`N` 步:`<step 描述>`。已完成步骤的结论:`<之前每步的 notify_text 拼接>`。可以调用 `list_md_files`/`read_md_file` 查证,完成后用纯文本给出这一步的结论和可以对外展示的进度说明。"
...
### 6.5 每次处理的 DeepSeek 调用次数

一次 peer 分组处理最坏情况下的调用次数 = 决策阶段(1 + 最多 `max_tool_calls_per_step`) + 步骤执行阶段(`max_plan_steps` × (1 + 最多 `max_tool_calls_per_step`)) + 汇总(1) + 记忆回填(1)。默认参数下上限是 `1+4 + 6×(1+4) + 1 + 1 = 37` 次;这是刻意的、可配置的成本/延迟上限,而不是无界循环,第 11 节会说明这与业内方案的取舍差异。

## 7. 文件读取沙箱工具(`agent/fsread`)

### 7.1 能力边界

只提供两个只读能力,不提供任何写、删、执行、目录穿越到 workspace 之外的能力:
...
- `agent/fsread`:合法读取、`..` 穿越拒绝、绝对路径拒绝、符号链接逃逸拒绝、非 `.md` 后缀拒绝、大文件截断,纯 Go 单测。
...
- `agent/engine`:注入实现同一接口的 fake `ChatCompleter`(不经 HTTP),按脚本化的多轮响应验证:直接回答分支、计划分支的步骤顺序与进度通知时机、工具调用次数上限、超过 `max_plan_steps` 时的截断行为,纯 Go 单测。
...
| 工具生态 | 仅一类只读 Markdown 文件工具 | 网页搜索、代码执行、数据库查询、调用其它内部/外部 API、子 Agent 调用等丰富工具集,且多为标准化协议(如 MCP) | 按需扩展工具集,优先考虑接入 MCP 协议而不是每个工具各写一套 schema |
...
| 工具调用标准化 | 自定义 3 个工具、内嵌在 DeepSeek 请求里,没有走通用协议 | 越来越多方案统一用 MCP(Model Context Protocol)描述和暴露工具 | 工具数量增长到一定规模后,评估把 `agent/fsread` 等工具改造成标准 MCP server,而不是继续手写 `tools` schema |
