...
这是本方案里最核心的部分,对应需求"Agent 可以直接回答,也可以指定计划,多步执行,每步执行完会发 Markdown 消息告知"。
...
3. 模型最终返回的纯文本被同时当作"这一步的结论"(喂给下一步的上下文)和"进度通知文本"(`notify_text`),通过调用方传入的 `Notifier` 立即发送成一条 Markdown 聊天消息(`MESSAGE_TYPE_MARKDOWN`,对应需求"每步执行完会发 Markdown 消息告知")——**不等全部步骤跑完再补发**,是执行到哪一步就通知到哪一步。
