...
/**
 * Register a new user and land on the app.
 * `layout` 可选：在启动模态框上手动选择布局偏好（auto/desktop/mobile），
...
export async function register(
  ...
  await page.goto('/app/');
  await ensureStartupPrefs(page, layout);
...
/**
 * 首次访问会弹出启动偏好弹层（当前只剩布局选择）；可选指定布局后确认进入。
 * 已经有登录态时不会弹出，此时直接返回。
 */
export async function ensureStartupPrefs(
  page: Page,
  layout?: 'auto' | 'desktop' | 'mobile',
) {
  const modal = page.locator('#modal-overlay:not(.hidden)');
  try {
    await page.locator('#layout-confirm-btn').waitFor({ state: 'visible', timeout: 1000 });
  } catch (_) {
  ...
  await page.click('#layout-confirm-btn');
...
export async function addFriend(page1: Page, page2: Page, username2: string) {
  ...
  // 同一页面可能连续添加多个好友；旧 toast 的 4s 定时器在浏览器主线程繁忙时可能延后，
  // 不能拿“等待退场”当成本次写入的前置条件。显式清理已验证过的旧提示，再观察新提示，
  // 业务状态仍由接收方请求列表和接受结果交叉验证。
  await requestSentToast.evaluateAll((toasts) => toasts.forEach((toast) => toast.remove()));
  await expect(requestSentToast).toHaveCount(0);
  ...
  await friendAddedToast.evaluateAll((toasts) => toasts.forEach((toast) => toast.remove()));
  await expect(friendAddedToast).toHaveCount(0);
