...
+ 修改代码后，无论在本地、远程沙箱、CI 还是任何其它环境，都必须从仓库根目录完整运行 `./tools/run_all_tests.sh` 全量测试，不得以环境受限为由跳过或只跑部分测试；运行完成后必须把每一步（unit、integration、E2E、component）的通过 / 失败结果明确输出告诉用户。性能测试保持独立，涉及性能路径时还必须执行 `./tools/run_performance_tests.sh`。
...
+ 如果需要部署，除非用户明确说明是本地部署，否则一律使用 GitHub Actions（`.github/workflows/deploy.yml`，手动 `workflow_dispatch` 触发）；只有用户明确要求本地部署时，才登录对应服务器（当前实际部署 `ssh yimsg-se`，首尔）手动执行。线上服务器相互独立，部署脚本支持按同样步骤增加更多独立服务器，详见 `docs/deployment/部署方案.md`。
...
## 全量测试脚本说明
- `./tools/run_all_tests.sh` 会准备依赖和构建，按 unit → integration → E2E → component 执行正确性测试；`./tools/run_performance_tests.sh` 是独立性能门禁。
