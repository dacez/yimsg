...
describe('uikit public surface', () => {
  ...
  it('workspace build script builds the standalone uikit bundle', () => {
    const pkg = JSON.parse(readFileSync(new URL('../../package.json', import.meta.url), 'utf8')) as {
      name?: string;
      scripts?: Record<string, string>;
    };

    expect(pkg.name).toBe('@yimsg/uikit');
    expect(pkg.scripts?.build).toContain('vite build --config vite.config.ts');
  });
