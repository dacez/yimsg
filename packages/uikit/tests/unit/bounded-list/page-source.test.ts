...
//   A sdkPageSource 透传 / B localPageSource 切片 / C 边界与非法输入 / D 大数据量。

import { describe, expect, it, vi } from 'vitest';
import { localPageSource, sdkPageSource } from '../../../src/app/bounded-list/page-source';
...
describe('PageSource / A sdkPageSource', () => {
...
describe('PageSource / D 大数据量', () => {
  it('D1 全量 1000 条时任意时刻窗口内条目数不超过 pageSize × maxPages', async () => {
    ...
    for (let i = 0; i < 30; i++) {
      const page = await source.fetch({ cursor: window.cursorFor('forward'), backward: false, limit: pageSize, query: undefined });
...
describe('localPageSource / E 新鲜端', () => {
  it('E1 order=desc（默认）时 reset 取 entries 最前面一页', async () => {
    const source = localPageSource<Item, void>({ loadAll: async () => makeItems(10) });
  ...
  it('E2 order=asc 时 reset 取最后一页，并从那里向前续翻', async () => {
    // 早前 reset 写死取最前面一页，与 order:'asc'（新鲜端在尾部）搭配会静默取错端：
  ...
  it('E3 order=asc 且总量不足一页时返回全部，两端都没有更多', async () => {
    const source = localPageSource<Item, void>({
