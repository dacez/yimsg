...
| `reset` | `(opts?: { query?: Q; pinEdge?: boolean }) => Promise<void>` | 清空窗口、清 `stale` / `pendingCount`、无游标拉首页重建。`pinEdge` 默认 `true`：渲染后把滚动摁到 `freshEdge`。传 `query` 同时更新查询条件。 | 递增 `requestId`，后发 reset 胜出；请求在飞期间的本地 upsert / patch / remove 按 identity 记录最终态，在首页候选窗口上重放后再提交（overlay 是有界 Map，硬预算见 §13.4）。返回 Promise 永远 resolve。 |
| `loadMore` | `(dir: Direction) => Promise<void>` | 正常情况下按该方向可信边界游标续翻一页，超限整页裁剪。显式调用视为「用户主动重试」，会解除该方向的自动续翻暂停（§13.3）。 | live eviction 已使该端游标失效时，不读旧游标，改发起 staged 权威 reconcile（与其它入口共享同一个在途 Promise）。普通分页期间的本地最终态在返回页并入后重放，只重放目标窗口**已经追平新鲜端**的 `upsert`；无法完整重放时丢弃该页、封锁两端游标并进入 stale / 显式 staged reconcile，但不把 `state.failed` 置真，也不自动循环。返回 Promise 永远 resolve。 |
...
| `upsertLocal` | `(item: T) => void` | 本端产生的新条目并入新鲜端，经 `normalize`。**前提是窗口已经追平新鲜端**（该端 `hasMore` 已为 `false`）：只有这时才能确定这条新条目和已加载内容的真实相邻关系。窗口为空时自建一页。 | 窗口尚未追平新鲜端时**不会**猜测位置并入 DOM，只记入 mutation overlay 并点亮一次待处理提示，等下一次真正追平新鲜端的 reset / reconcile / 续翻把它带回真实位置。已追平时同步跨页去重并裁到硬预算；eviction 后立即封锁被裁端游标，并作废基于旧边界的在飞普通分页，同时仍记入 overlay 供权威请求重放。 |
| `patch` | `(id: string, update: (item: T) => T) => boolean` | 就地更新窗口内该身份的全部条目，页结构与边界游标不变。返回是否命中。命中才重渲并触发 `onItemsChanged`。窗口里没有但命中一条还未追平新鲜端的 pending `upsert` 时，直接改写这条待重放的最终态，同样返回 `true`。 | 同步；使该 identity 的在飞定向刷新失效。任一数据请求在飞或容量游标已失效时，同时记录 replace overlay（对 pending upsert 则原地更新，不降级）。 |
| `removeLocal` | `(id: string) => boolean` | 就地删除窗口内该身份的条目，剩余条目自然往上补齐。返回是否命中。命中时**只精确摘掉这一个身份**的选中态。窗口里没有但命中一条还未追平新鲜端的 pending `upsert` 时，同样返回 `true` 并记一条 `remove`（不是直接撤销待办：权威页在途时可能碰巧携带同一身份的旧记录，只有留下 `remove` 才能在重放时一并挡住）。 | 同步；使该 identity 的在飞定向刷新失效，并在任一数据请求期间记录 remove overlay。共享 `store` 时其它实例、`pinnedItems`、已被裁剪出窗口的选中项不受影响。 |
...
多个 reset 并发时，generation 守卫只允许最新世代发布共享 `entries` 缓存；旧世代自己的首页 snapshot 仍可返回给上层，由 BoundedList 的 `requestId` 丢弃。若调用方需要展示加载进度（例如「已加载 N 人」），在 `loadAll` 内部自行通过闭包上报即可，不需要经由 `PageSource` 接口透传。
...
`BoundedList.upsertLocal` 只在窗口**已经**追平新鲜端（该端 `hasMore` 已为 `false`）时才调用 `mergeLive`；否则把这条本端新增记入 mutation overlay（§13.4）并只点亮一次待处理提示，不动 DOM。`replayPendingMutations` 重放 overlay 时同样逐条检查目标窗口在那一刻是否已追平新鲜端：只有满足条件的 `upsert` 才会被重放并从 overlay 摘除，不满足条件的原样保留，等下一次真正追平时再重放——不会因为某一次重放没有条件应用就被悄悄丢弃。
...
2. 后台发 `cursor: undefined` 的权威首页请求；同一时刻只允许一个 reconcile 在飞，显式追平（`loadMore`、提示条重试）一律复用在途请求的同一个 Promise，并取消尚未执行的自动调度帧——不再制造第二个并发请求。
3. 所有数据请求（普通 reset / loadMore / 容量 reconcile）期间的 `upsertLocal` / `patch` / `removeLocal` 都按 identity 记录最终态；首页响应在独立 `PageWindow` 中完成 normalize、单页校验和 mutation 重放后原子替换，分页响应在并入当前窗口后重放，均不得覆盖本地最终态。重放 `upsert` 前会先确认目标窗口在那一刻已经追平新鲜端（见 §8），不满足条件的原样留在 overlay 里等下一次机会。
4. 请求失败、响应校验失败或被更新世代取代时，当前 capped rows 不被清空；有效失败进入 `failed + stale` 并保留 retry。
5. mutation overlay 是按 identity 合并的有界 Map，不是会丢语义的操作 FIFO；被重放（应用到某个目标窗口）的条目立即从 Map 里摘除，未满足重放条件的条目原样保留，不随意被清空。唯一 identity 数超过 `pageSize×maxPages` 时只静默丢最旧的一条——这个数量级需要一次权威请求的在飞窗口内发生这么多个不同 identity 的本地写入，现实中不会发生，用一次可忽略的极端退化换掉「显式失败 + 强制新快照」的复杂度，不会让 reset / reconcile 因此失败。所有路径都**绝不在响应处理循环里自动重取**。
