...
| `reset` | `(opts?: { query?: Q; pinEdge?: boolean }) => Promise<void>` | 清空窗口、清 `stale` / `pendingCount`、无游标拉首页重建。`pinEdge` 默认 `true`：渲染后把滚动摁到 `freshEdge`。传 `query` 同时更新查询条件。 | 推进窗口世代，后发 reset 胜出；请求在飞期间的本地 upsert / patch / remove 按 identity 记录最终态，在首页候选窗口上重放后再提交（overlay 是有界 Map，硬预算见 §13.4）。返回 Promise 永远 resolve。 |
