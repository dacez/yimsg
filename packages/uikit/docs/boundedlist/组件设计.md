...
按这个口径，以下类型只在本目录内部使用，因此**不导出**：`BoundedListText`、`Direction`、
`FreshEdge`、`SelectionConfig`、`RegisterBoundedList`、`LocalPageSourceOptions`、`ToggleResult`。
...
| `loadMore` | `(dir: Direction) => Promise<void>` | 正常情况下按该方向可信边界游标续翻一页，超限整页裁剪。显式调用视为「用户主动重试」，会解除该方向的自动续翻暂停（§13.3）。 | live eviction 已使该端游标失效时，不读旧游标，改发起 staged 权威 reconcile（与其它入口共享同一个在途 Promise）。普通分页期间的本地最终态在返回页并入后重放：`replace` / `remove` 无条件应用，`upsert` 只在窗口里已有该身份时按 replace 应用（见 §13.4）。返回 Promise 永远 resolve。 |
...
- `hasMoreBackward` / `hasMoreForward` 在组件里是**只读**的，语义是「**该端现在能不能续翻**」。正常分页取自 `PageWindow`；live eviction 使某端游标失效时，该端立即对外屏蔽为 `false`，从而阻止渲染引擎用旧游标自动续翻。
- 因此 `hasMoreBackward === false` **不等于**「那一端没有数据了」：它还包含「有数据，但边界游标暂时不可信」这种情况。「没有更多了」边界提示因此不看它，而是看窗口自己的账（渲染状态里的 `atBackwardEnd` / `atForwardEnd`，见 §9.1）。两者混用会在 live 硬裁剪之后错报「已到最早」，而且那之后一旦追平失败，这条错误提示就再没有东西会把它改回来。
...
interface BoundedStreamWindowRenderState<T> {
  ...
  readonly hasMoreBackward?: boolean; // 现在能不能续翻 → 驱动触界检测
  readonly hasMoreForward?: boolean;
  readonly atBackwardEnd?: boolean;   // 确认到达数据尽头 → 驱动边界提示；默认 !hasMoreBackward
...
| `keydown` | `scrollElement` | `↑` / `↓` 移动焦点行（首次 `↓` 落在第 0 条、首次 `↑` 落在最后一条；越界时改为触发对应方向的 `loadBackward` / `loadForward` 且**不移动焦点**），`Enter` / `Space` 激活当前聚焦行 → `onInteract(key, ev)`（与点击同一个回调，不区分来源）。方向键与激活键会 `preventDefault()`；其它按键完全不消费。 |
...
注意触界检测与边界提示用的是**两个不同信号**：触界看 `hasMore*`（能不能续翻），边界提示看 `atBackwardEnd` / `atForwardEnd`（是不是确认到尽头）。失效游标期间两者都不该动作——既不自动续翻，也不显示「没有更多了」。
...
2. 后台发 `cursor: undefined` 的权威首页请求；同一时刻只允许一个权威首页请求在飞，显式追平（`loadMore`、提示条重试）一律复用在途请求的同一个 Promise，并取消尚未执行的自动调度帧——不再制造第二个并发请求。**公开 `reset` 与 reconcile 在这件事上完全对等**：两者都在发起时把自己的 Promise 登记进在飞记录，因此 reset 在飞期间的一次容量追平会合并到那次 reset，而不是递增 `requestId` 把它作废再发一个（与下面第 8 条「权威请求已在飞时不额外安排第二次追平」是同一条约定）。
...
**overlay 按「是否依赖位置」分成两类，重放路径也因此分成两条。**
