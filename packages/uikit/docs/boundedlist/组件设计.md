...
flowchart LR
    ...
    SW -. "loadBackward / loadForward / onInteract / onScroll" .-> BL["BoundedList"]
    BL --> PS
...
| `backwardBoundary` | 固定显示在头部 | `!hasMoreBackward` |
| `forwardBoundary` | 固定显示在尾部 | `!hasMoreForward` |
...
interface BoundedListState {
  ...
  readonly loading: boolean;       // loadingBackward || loadingForward || staged reconcile
  readonly loadingBackward: boolean; // 头部方向正在加载
  readonly loadingForward: boolean;  // 尾部方向正在加载
  readonly hasMoreBackward: boolean;
  readonly hasMoreForward: boolean;
...
| `pinnedItems` | `() => readonly T[]` | 否 | 钉在窗口条目**头部**、不进数据窗口的纯展示条目（会话列表的「空会话占位」、`@` 选择器的「所有人」）。参与渲染、参与 `renderItem` 的 `index` / `previous` 链、参与点击命中，但**不计入 `state.count`**，也不参与分页与裁剪。 |
...
| `reset` | `(opts?: { query?: Q; pinEdge?: boolean }) => Promise<void>` | 清空窗口、清 `stale` / `pendingCount`、无游标拉首页重建。`pinEdge` 默认 `true`：渲染后把滚动摁到 `freshEdge`。传 `query` 同时更新查询条件。 | 递增 `requestId`，后发 reset 胜出；请求在飞期间的本地 patch / remove 按 identity 记录最终态，在首页候选窗口上重放后再提交（overlay 是有界 Map，硬预算见 §13.4）。返回 Promise 永远 resolve。 |
| `loadMore` | `(dir: Direction) => Promise<void>` | 正常情况下按该方向可信边界游标续翻一页，超限整页裁剪。显式调用视为「用户主动重试」，会解除该方向的自动续翻暂停（§13.3）。 | live eviction 已使该端游标失效时，不读旧游标，改发起 staged 权威 reconcile（与其它入口共享同一个在途 Promise）。普通分页期间的本地最终态（`replace` / `remove`）在返回页并入后无条件重放；它们不会让窗口变大，因此不涉及裁剪、游标失效或二次追平。返回 Promise 永远 resolve。 |
...
| `upsertLocal` | `(item: T) => void` | 本端产生的新条目并入新鲜端，经 `normalize`。**前提是窗口已经追平新鲜端**（该端 `hasMore` 已为 `false`）：只有这时才能确定这条新条目和已加载内容的真实相邻关系。窗口为空时自建一页。 | 窗口尚未追平新鲜端时**不会**猜测位置并入 DOM，只点亮一次待处理提示，由用户或贴边追平从服务端拿回真实顺序。已追平时无条件并入（本端发送要立即可见），同步跨页去重并裁到硬预算；eviction 后立即封锁被裁端游标，并作废基于旧边界的在飞普通分页。权威请求在飞时并入照做，但同时点亮提示条——那次响应会整体替换窗口，本端条目以服务端顺序为准，见 §13.4。 |
...
- `hasMoreBackward` / `hasMoreForward` 在组件里是**只读**的。正常分页取自 `PageWindow`；live eviction 使某端游标失效时，该端立即对外屏蔽为 `false`，从而阻止渲染引擎用旧游标自动续翻。
...
| 只读属性 | 类型 | 语义 |
|---|---|---|
| `hasMoreBackward` / `hasMoreForward` | `boolean` | 只有 getter，没有 setter。 |
| `loaded` | `boolean` | `pages.length > 0`。注意**空首页不算已加载**（组件层因此另用 `firstLoadDone`）。 |
| `total` | `number` | `-1` 表示未知。 |
| `items` | `T[]` | 每次调用都重新 flatten 拼接一个新数组。 |
| `count` | `number` | 各页条目数之和。 |
| `backwardCursor` | `string` | 窗口 backward 端的续翻锚点。与 `pages` **解耦维护**：页可能因跨页去重 / `removeLocal` 被清空而整页回收，但「窗口边界在哪里」不因此改变。从未加载过才是 `''`。 |
...
| `keydown` | `scrollElement` | `↑` / `↓` 移动焦点行（首次 `↓` 落在第 0 条、首次 `↑` 落在最后一条；越界时改为触发对应方向的 `loadBackward` / `loadForward` 且**不移动焦点**），`Enter` / `Space` 激活当前聚焦行 → `onInteract(key, ev, true)`。方向键与激活键会 `preventDefault()`；其它按键完全不消费。 |
...
| # | 路径 | 触发条件 | 效果 |
|---|---|---|---|
| ① | 用户自己滚回新鲜端 | 每个滚动帧检查 `stale && !loadingBackward && !loadingForward && atFreshEdge` | 已加载窗口统一 staged reconcile，保留当前 DOM 到新首页原子提交；首屏尚未落定时走 `reset` |
...
scrollTop ≤ reachPx                且 hasMoreBackward → loadBackward()
maxScrollTop - scrollTop ≤ reachPx 且 hasMoreForward  → loadForward()
```

引擎只用 `hasMore*` **快照**粗滤，真正的并发与终止守卫在 `loadMore` 内部（实时读 `loadingBackward` / `loadingForward` / `hasMore*`）。live eviction 后失效端对外 `hasMore=false`，因此引擎不会自动携旧 cursor 续翻；宿主显式请求该方向时也会转入无游标 staged reconcile。
...
另外，某方向没有可用边界游标（空串）时 `loadMore` 直接返回不发请求：空串不是 reset 语义，发出去只会让服务端按未定义行为处理。窗口为空但服务端给过真实游标时，那个锚点仍然保留并继续可用（§8）。
...
- 公开 `reset` 与 reconcile 开始时都会把 `loadingBackward` / `loadingForward` 归零，因此被丢弃的 `loadMore` 不会留下悬空加载标志。
...
**overlay 只装与位置无关的最终态。** `replace`（该身份若在窗口里就改成这个值）和 `remove`
（该身份从窗口里去掉）都是幂等的、与重放顺序无关、命中不到就是空操作，因此可以无条件重放到
任何窗口上，也不会让窗口变大——调用方不必处理裁剪、游标失效或二次追平。

overlay 曾经还有第三类 `upsert`（本端新增要在权威响应后重新并入）。它和另外两种根本不同：
新增必须知道自己和已加载内容的相邻关系，为此牵出三条规则——逐条判定「目标窗口是否已追平新鲜
端」、Map 插入顺序即重放顺序（于是 `replace` 不能移位、`upsert` / `remove` 必须移到尾部）、
`replace` 撞上已有 `upsert` 要降级回 `upsert`——占了整个 overlay 的绝大部分复杂度。现在不再
猜位置：`upsertLocal` 在窗口已追平新鲜端时照常并入（本端发送要立即可见），但**不试图让它活过
一次权威窗口替换**；权威请求在飞时并入的同时点亮提示条，响应落地后以服务端顺序为准。

> 这条取舍有一个已知的可见代价：权威请求在飞期间本端并入的条目，会在响应落地时先消失，
> 若此时贴在新鲜端则提示条自动追平再取一次、服务端已收录后重新出现，形成一次闪动。
> 触发条件是「窗口已满且刚发生过容量裁剪，紧接着又有本端新增」。
...
4. 头部：!hasMoreBackward && backwardBoundaryText → 边界提示
        否则 loadingBackward && loadingText  → 加载提示
...
6. 尾部：!hasMoreForward && forwardBoundaryText → 边界提示
        否则 loadingForward && loadingText    → 加载提示
