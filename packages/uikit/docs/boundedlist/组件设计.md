...
> 主要对照：`packages/uikit/src/app/bounded-list/`（`bounded-list.ts`、`page-window.ts`、`overlay.ts`、`renderer.ts`、`page-source.ts`、`selection.ts`、`update-pill.ts`、`frame.ts`、`deep-equal.ts`、`registry.ts`、`types.ts`、`index.ts`）。
> 最后复核：2026-08-13。
...
| 辅助 | `update-pill.ts`、`frame.ts`、`deep-equal.ts`、`registry.ts` | 提示条、帧调度、结构比较、宿主注册契约 | 部分 | 否 |
...
```mermaid
flowchart TD
    Host["宿主视图"] -->|reset / setQuery / loadMore| BL["BoundedList"]
    Host -->|invalidate| BL
...
| `serverPageSource(fetch, map)` | 函数 | 服务端游标分页数据源 |
| `localPageSource(options)` | 函数 | 本地全量切片数据源 |
...
不导出：`PageWindow`、`LocalOverlay`、`ListRenderer`（实现分层），以及 `BoundedListText`、`Edge`、`DisplayOrder`、`SelectionConfig`、`RegisterBoundedList` 等只作为已导出类型组成部分出现的类型（结构类型下调用方无需单独 import）。
...
| `Edge` | `'head'` / `'tail'` | 数组与 DOM 的一端 |

`order` 只决定一件事：**新鲜端在哪一端**。`desc → head`，`asc → tail`。转换只在构造时发生一次，此后组件内部只用 `Edge`。

三套方向词汇的对应关系（永远成立，没有例外）：

| 组件内部 | wire 协议 | `PageLoadResult` |
|---|---|---|
| `head` | `backward: true` | `hasMoreHead` |
| `tail` | `backward: false` | `hasMoreTail` |

调用方从 SDK `PageInfo` 映射时按 `hasMoreBackward → hasMoreHead`、`hasMoreForward → hasMoreTail` 一次性转换，**不随展示序变化**。
...
interface FetchPageRequest<Q> {
  ...
  backward: boolean;      // true = 朝 head 端翻
...
interface PageLoadResult<T> {
  items: readonly T[];
  startCursor: string;    // 这一页 head 侧的边界游标
  endCursor: string;      // 这一页 tail 侧的边界游标
  hasMoreHead: boolean;
  hasMoreTail: boolean;
...
| `empty()` | 渲染序列为空且当前查询条件等于 `initialQuery` |
| `emptyFiltered()` | 渲染序列为空且查询条件已变化；缺省时回退到 `empty()` |
| `headBoundary()` / `tailBoundary()` | 该端 `hasMore` 为 false 且列表非空 |
| `updatePill()` | `stale` 为真；**不提供则不显示提示条** |
| `error(err)` | 首屏失败且渲染序列为空；缺省时回退到空态文案 |
| `retry()` | 首屏失败时提示条的文案；**不提供则失败后不显示重试入口** |
...
| `hasMoreHead` / `hasMoreTail` / `hasMore(edge)` | 该端还有没有数据 |
...
- `overlay.apply`：先把所有被记账的身份从窗口序列里摘掉，再把 `put` 的条目按记账序落到新鲜端（`head` 端时最后记账的排最前）。
- pinned 去重：窗口 / 本地层里已经出现的身份，pinned 一律让位（占位会话在真实条目落库后就不该继续钉在头部）。
...
| `contentElement` | 否 | 行的实际父节点；不提供时等于 `scrollElement` |
| `pillHost` | 否 | 提示条挂载点。缺省 = `scrollElement.parentElement`；`false` = 不创建提示条 DOM |
| `register` | 是 | 登记到宿主注册表并返回注销函数；不接入广播的一次性列表显式传 `standaloneList` |
| `isActive` | 否 | 列表当前是否可见；返回 false 时通知只点亮提示条 |
...
| `fetchByIdentity` | 否 | 定向刷新；不提供时带 `identities` 的通知只点亮提示条 |
...
| `stickyPx` | head: 4 / tail: 50 | 距新鲜端多少像素内算贴边 |
| `reachPx` | 160 | 距某端多少像素内触发自动续翻 |
| `settleFrames` | head: 1 / tail: 4 | 贴边校正连续执行多少帧（tail 端内容会异步增高） |

### 5.4 渲染、选中与查询

| 参数 | 说明 |
|---|---|
| `renderItem(item, ctx)` | 返回该行的**平级根节点数组**；第一个节点承载锚点键与 `role="option"` |
...
| `initialQuery` | 初始查询条件；同时是「当前是否处于过滤态」的比较基准 |

### 5.5 事件回调

| 回调 | 触发时机 |
|---|---|
| `onActivate(item, ev)` | 点击或键盘 Enter / Space 激活某行（多选模式下不触发） |
| `onSelectionChange(snapshot)` | 选中集变化，`items` 与渲染序列同序 |
| `onLoadStateChange(state)` | 加载状态、条目数、提示条状态变化 |
| `onItemsChanged(items)` | 渲染序列变化，**含 pinnedItems 与本地层** |
| `onError(error, phase)` | `phase ∈ 'reset' \| 'head' \| 'tail' \| 'refresh'`；不提供时降级为 `console.warn` |
...
| `setQuery` | `(query, opts?: { debounceMs? }) => void` | 改查询条件并重拉；默认防抖 300ms，`<= 0` 立即执行 |
...
### 8.1 serverPageSource

```ts
serverPageSource(fetch, map)
```

`fetch` 原样透传 `FetchPageRequest`，`map` 把 SDK 响应整理成 `PageLoadResult`。游标原样透传，不解析、不构造。

### 8.2 localPageSource

一次性拉全量到内存，之后所有续翻都是对内存数组的下标切片。**只用于「刻意选择在客户端过滤 / 排序」的低频弹窗**（提及群成员、添加群成员候选）。
...
| `order` | 必须与所属 BoundedList 的 `order` 一致，默认 `'desc'` |
...
- `order: 'asc'` 时首页取**最后一页**（新鲜端在尾部），与所属列表不一致会静默取错端。

## 9. ListRenderer 渲染引擎

只做三件事：有键 DOM 协调、滚动保持、事件。不认识分页、通知、选中态。

### 9.1 渲染状态

`render(state)` 的 `ListRenderState` 全部字段必填，由 `bounded-list.ts` 一次性填满：`items`、`loaded`、`hasMoreHead/Tail`、`loadingHead/Tail`、`emptyText`、`errorText`、`loadingText`、`headBoundaryText`、`tailBoundaryText`、`loadMore(edge)`、`renderItem`、`keyOf`、`reuseUnchangedRows`、`revisionOf`。
...
| 否则跑 `renderItem`，若候选 DOM 与旧节点完全等价 | 复用旧节点（显式 `render()` 时额外要求 item 也相等，便于宿主刷新行内自有监听） |
| 其它 | 用新节点替换 |

`revisionOf` 是「会影响 `renderItem` 输出、但不在 item 里」的上下文签名，当前包含：是否首行、上一条身份、选中、可选。**只记「是否首行」而非精确下标**——精确下标入签名会让任何头部插入使整窗口失去复用。

协调按位置进行：逐个 `insertBefore` 期望节点，再删掉多余的尾部节点。

### 9.3 滚动保持

渲染前捕获锚点（第一条底边仍在视口顶以下的行 + 它相对视口顶的偏移），渲染后按同一身份的新位置校正 `scrollTop`。拿不到锚点（空列表、首屏、宿主 DOM 不支持 `getBoundingClientRect`）时退回「恢复渲染前的绝对 `scrollTop`」。两者是「精确」与「兜底」的关系，不叠加。

### 9.4 自持的 DOM 事件

| 事件 | 挂在哪 | 用途 |
|---|---|---|
| `scroll` | scrollElement | 同步回调 `onScrollImmediate`，帧合并后回调 `onScroll` + 触界检测 |
| `pointerdown` / `pointerup` / `pointercancel` | scrollElement **和 window** | 指针按下期间推迟 DOM 重建，避免点击被吃掉；window 上是「指针在列表外抬起」的兜底 |
| `keydown` | scrollElement | 上下键移动高亮、Enter / Space 激活、越界时请求续翻 |
| `click` | contentElement | 事件委托，沿 `parentElement` 上溯找交互键 |
| `load`（捕获） | contentElement | 图片等异步增高内容加载完成 |
...
| `bsw-row-focused`（class） | 键盘高亮行 | 键盘导航视觉反馈 |

组件另外在行首节点写 `role="option"`，有选中态时写 `aria-selected`；在 `scrollElement` 上写 `tabindex="0"`、`role="listbox"`，多选时写 `aria-multiselectable="true"`，并在 `dispose()` 时**还原宿主原值**。
...
| 模块 | 接口 | 要点 |
|---|---|---|
| `update-pill.ts` | `createUpdatePill(host \| false, onClick)` → `{ setVisible(visible, text?), dispose() }` | `host === false` 时返回空操作句柄，不创建任何 DOM |
...
| `deep-equal.ts` | `valuesEquivalent(a, b)` | 结构相等：环引用用 WeakMap 记账，Date / TypedArray 按值比，原型不同判不等，深度上限 8。**不能退化成 `JSON.stringify`**（键序不同会把查询误判成已过滤） |
| `registry.ts` | `BoundedListController` 类型、`standaloneList` 常量 | 注册表实体由宿主持有；`standaloneList` 让「不注册」成为可 grep 的显式决定 |

## 12. 核心行为规则

### 12.1 请求单飞

`loading: 'first' | 'head' | 'tail' | null` 既是状态也是锁。
...
- **只有 `decide()` / `setStale()` 写 `stale`。** 「正在追平」不是「有没看到的更新」；在请求在飞时提前点亮提示条，就会在响应落地后立刻熄灭，用户看到的是闪一下（见 §12.7）。
- 三个入口会重新叫醒决策：`invalidate()`、请求落地、用户滚回新鲜端。
- 追平（`refresh`）保留当前窗口，落地时原子替换，所以追平期间画面不闪空。
- `dirty` 在请求发出时清零，请求期间到达的通知会重新置位，落地后再决策一次。
...
| `stale` 且提供了 `updatePill()` | 显示 | 贴回新鲜端并立刻追平 |
| 其它 | 隐藏 | — |

提示条自动熄灭的两条路径：追平成功；用户自己滚回新鲜端（滚动帧发现贴边且 `dirty` → 重新决策 → 追平）。
...
| 阶段 | 表现 |
|---|---|
| `reset`（首页） | `failed = true`、`loaded = true`；渲染序列为空时显示错误态，提示条变重试入口；**不自动重试**（等用户点击或下一次 `invalidate`），避免请求风暴 |
| `head` / `tail`（续翻） | 该端加入 `blocked`，窗口不变 |
...
| I9 | `dispose()` 之后监听全部归零、提示条 DOM 与 a11y 属性还原、排队的帧任务不再触碰 DOM、注册表不保留该实例 |

### 13.2 宿主约定

1. `scrollElement` 必须有确定高度与 `overflow-y: auto`；两个同时展示的分页区域不能共享同一个滚动容器。
2. 不要设置 `overflow-anchor: none`：浏览器原生锚点是图片、头像异步增高的兜底。
3. `contentElement` 的子节点由组件全权管理；业务监听放在 `renderItem` 返回的节点上，或交给组件的事件委托。
