...
| 阶段 | 触发 | 表现 |
|---|---|---|
| `reset`（清空重拉首页） | `reset()` / `setQuery()` / 首屏 | `failed = true`、`loaded = true`；渲染序列为空时显示错误态，提示条变重试入口；**不自动重试**（等用户点击或下一次 `invalidate`），避免请求风暴 |
| `catchup`（保留窗口重拉首页） | 追平决策、点提示条 | 状态同上，但**窗口里的旧内容还完整摆着**，用户看到的不是空列表 |
