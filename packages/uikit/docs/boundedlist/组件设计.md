...
| `reset` | `(opts?: { query?: Q; pinEdge?: boolean }) => Promise<void>` | 清空窗口、熄灭 `stale`、无游标拉首页重建。`pinEdge` 默认 `true`：渲染后把滚动摁到 `freshEdge`。传 `query` 同时更新查询条件。 | 推进窗口世代，后发 reset 胜出；请求在飞期间的本地 upsert / patch / remove 按 identity 记录最终态，在首页候选窗口上重放后再提交（overlay 是有界 Map，硬预算见 §13.4）。返回 Promise 永远 resolve。 |
...
| ② | 翻页翻到新鲜端的尽头 | `loadMore(freshDirection)` 之后该端 `hasMore` 收敛为 `false` | `stale` 熄灭 |
...
- `loadMore` **捕获**当前世代但不推进：被后续 `reset` / reconcile 取代后结果整体丢弃；两个可信方向可以同时在飞。请求期间发生的本地 mutation 会在返回页并入后按 identity 重放，返回页中的旧值不能覆盖本地最终态。
...
4. 头部：atBackwardEnd && backwardBoundaryText → 边界提示（atBackwardEnd 缺省时退化为
        !hasMoreBackward；两者的区别见 §6 与 §9.1）
...
6. 尾部：atForwardEnd && forwardBoundaryText → 边界提示
