...
> 主要对照：`packages/uikit/src/app/bounded-list/`（`bounded-list.ts`、`page-window.ts`、`overlay.ts`、`renderer.ts`、`page-source.ts`、`selection.ts`、`update-pill.ts`、`frame.ts`、`deep-equal.ts`、`types.ts`、`index.ts`）。
> 最后复核：2026-08-16。
...
| 辅助 | `update-pill.ts`、`frame.ts`、`deep-equal.ts` | 提示条、帧调度、结构比较 | 部分 | 否 |
...
| `reachPx` | 160 | 距某端多少像素内触发自动续翻；传负值等价于关掉自动补页（测试用） |

贴边阈值（head 4px / tail 50px）与贴边校正帧数（head 1 帧 / tail 4 帧）由新鲜端直接决定，
...
`backward → head`、`forward → tail` 的方向转换**全项目只在这个函数里发生一次**。
