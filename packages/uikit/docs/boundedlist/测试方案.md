...
八个测试文件、共 **442 个用例**。每个文件内部按字母段分类，用例 id（`A1`、`C7`…）写在用例名开头，便于在文档与代码之间互相指认。
...
| L | 辅助导出（10 例） | `catchUpAtEdge` 三态 + 短路 + fire-and-forget、`createFrameScheduler` 的合并 / 重排 / cancel / cancel 后重排 / 未调度时 cancel / 无 rAF 时同步执行 |
...
同轮结果为 8 个文件、442 项全部通过。未覆盖行集中在 staged reconcile / overlay、有键 DOM 协调的少量防御分支、极端裁剪分支和运行时降级路径；不能沿用“逐文件 100%”的旧结论。`index.ts` 与 `types.ts` 没有可执行语句（纯再导出 / 纯类型）。
...
| BL-BUG-19 | `localPageSource.loadAll` 的 `onProgress` 无法从 `fetch` 透传，进度能力实际不可用 | 当时修复为 `FetchPageRequest` 增加可选 `onProgress`，组件在 `reset` 时按 `onLoadProgress` 注入；后续复核发现生产代码从未配置 `onLoadProgress`，两处调用方一直用自己的闭包上报进度，这条链路是纯粹的死代码——已整体删除（`onLoadProgress`、`FetchPageRequest.onProgress`、`LocalPageSourceOptions.loadAll` 的 `onProgress` 参数），进度改为调用方在 `loadAll` 内部自行用闭包上报 | 已随删除一并移除对应用例，不再保留回归入口 |
...
| 编号 | 缺陷 | 修复方式 | 回归用例 |
|---|---|---|---|
| BL-BUG-26 | `mergeLive` 无条件把新鲜端 `hasMore` 置 `false`：窗口还没真正追平新鲜端（该端仍有未加载数据）时调用 `upsertLocal`，会把本端新增错误地拼接在一段旧历史后面，同时误关真正的续翻——消息列表场景下，用户向上翻历史时发送新消息即可复现，中间那段消息在下一次 reset 之前永久不可见 | `mergeLive` 不再替调用方改 `hasMore`，改由调用方保证前提；`upsertLocal` 增加 `reachesFreshEdge()` 守卫，未追平时只记入 mutation overlay、点亮待处理提示，不猜测位置硬并入；`replayPendingMutations` 同样逐条检查目标窗口是否已追平新鲜端才重放 `upsert`，不满足条件的原样保留等下一次机会；`patch` / `removeLocal` 相应扩展，能命中「窗口没有但存在于 pending upsert」的条目 | `page-window` E1/E1b、`bounded-list` C21/E12i/G1/G3 |
...
用例数与整体统计以 `tools/scripts/check_docs_consistency.sh` 的输出为准；本文 §4 的分项数量在用例增删时同步更新。当前通过 Vitest 实际执行核对为 442 项，其中 `bounded-list.test.ts` 为 189 项、`page-window.test.ts` 为 54 项、`stream-window.test.ts` 为 103 项。
