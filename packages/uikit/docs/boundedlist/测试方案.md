...
**② `reset` 默认 `pinEdge: true` 会把 `scrollTop` 摁到新鲜端。** 摁到顶之后，只要 `hasMoreBackward` 为 `true`，之后的每一次渲染都会触发 `backward` 续翻。所以只想观察某个单点行为的用例一律用 `reset({ pinEdge: false })` 再显式设置 `scrollTop = 500`。
