...
八个测试文件、共 **445 个用例**。每个文件内部按字母段分类，用例 id（`A1`、`C7`…）写在用例名开头，便于在文档与代码之间互相指认。
...
| 指标 | 结果 |
|---|---|
| Statements | **95.75%**（1173 / 1225） |
| Branches | **93.54%**（739 / 790） |
| Functions | **97.94%**（191 / 195） |
| Lines | **96.49%**（1020 / 1057） |

同轮结果为 8 个文件、445 项全部通过。未覆盖行集中在 staged reconcile / overlay、有键 DOM 协调的少量防御分支、极端裁剪分支和运行时降级路径；不能沿用“逐文件 100%”的旧结论。`index.ts` 与 `types.ts` 没有可执行语句（纯再导出 / 纯类型）。
...
- `BL-UNIT-BUG-001`～`BL-UNIT-BUG-025`：首轮单元评审发现的产品缺陷。
...
用例数与整体统计以 `tools/scripts/check_docs_consistency.sh` 的输出为准；本文 §4 的分项数量在用例增删时同步更新。当前通过 Vitest 实际执行核对为 445 项，其中 `bounded-list.test.ts` 为 191 项、`page-window.test.ts` 为 53 项、`stream-window.test.ts` 为 103 项。
