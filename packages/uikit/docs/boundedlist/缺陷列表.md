...
- 单元补强：`packages/uikit/tests/unit/bounded-list/` 当前基线为 7 个文件、417 项；单元分项与覆盖率见 [`测试方案.md`](测试方案.md)。
...
- 单元证据：`E12j refresh 先于旧分页返回时，新值不回退且删除项不复活`，并额外断言首个方向返回后 overlay 仍保留、双向分页全部完成后清零；当前 BoundedList 单元基线为 417 项。
