...
- 产品缺陷状态：`BL-BUG-001`～`BL-BUG-012` 共 12 条真实 Chromium 缺陷、`BL-UNIT-BUG-001`～`BL-UNIT-BUG-026` 共 26 条单元评审缺陷均已关闭，正确期望断言全部保留为普通回归测试。
- 测试基础设施：`BL-TEST-BUG-001`～`BL-TEST-BUG-003` 已关闭；并发方向用例改用可控双请求 gate，防抖用例改用暂停虚拟时钟后的 299+1ms 精确推进，宿主视图的轻量 DOM 替身补齐标准节点移动与删除语义。
- 单元补强：`packages/uikit/tests/unit/bounded-list/` 当前基线为 8 个文件、442 项；单元分项与覆盖率见 [`测试方案.md`](测试方案.md)。
...
- 单元证据：`E12j refresh 先于旧分页返回时，新值不回退且删除项不复活`，并额外断言首个方向返回后 overlay 仍保留、双向分页全部完成后清零；当前 BoundedList 单元基线为 442 项。
