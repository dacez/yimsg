...
| 添加群成员候选 | `views/chat/detail-panel.ts` | `localPageSource` | 用户 ID | `backward` |
| 提及群成员 | `views/group-member-picker.ts` | `localPageSource` | 用户 ID | `backward` |
...
| 本端产生或乐观插入条目 | `upsertLocal()` | 立即可见并落在新鲜端；窗口里已有同身份时等价于「移到新鲜端」（发消息后会话置顶）。不发请求、不点亮提示条 |
