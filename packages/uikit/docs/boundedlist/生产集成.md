...
| 场景 | 主要文件 | 数据源 | 身份键 | 新鲜端 |
|---|---|---|---|---|
| 会话列表 | `views/chat/conversation-list.ts` | `sdkPageSource` | `conversationIdentity` | `head` |
| 消息列表 | `views/chat/message-list.ts` | `sdkPageSource` | `messageKey` | `tail` |
| 通讯录好友 | `views/contacts.ts` | `sdkPageSource` | `contactIdentity` | `head` |
| 待处理好友请求 | `views/contacts.ts` | `sdkPageSource` | `contactIdentity` | `head` |
| 已发出好友请求 | `views/contacts.ts` | `sdkPageSource` | `contactIdentity` | `head` |
| 群成员详情 | `views/chat/detail-panel.ts` | `sdkPageSource` | 用户 ID | `head` |
| 转发会话候选 | `views/chat/forward.ts` | `sdkPageSource` | 会话 key | `head` |
| 转发通讯录候选 | `views/chat/forward.ts` | `sdkPageSource` | 会话 key | `head` |
| 建群候选 | `views/contacts.ts` | `sdkPageSource` | 好友 UID | `head` |
...
| 会话内消息搜索 | `views/chat/message-search.ts` | `sdkPageSource` | `messageKey` | `tail` |
...
- 服务端已经提供不透明 keyset 游标时使用 `sdkPageSource`，原样透传 `cursor`、`backward` 和 `limit`；`backward → head` 的方向映射由它统一完成，调用方只指出条目在响应的哪个字段。
