...
> 入口关系：上级索引见 [`README.md`](../README.md)；本文是 UIKit 列表渲染**原理与目标态契约**的单一事实源，`UI设计方案.md` §8 保留概述性表格，细节以本文为准。§1–§6 是组件契约（目标态），§7 是当前代码逐文件对照（现状），§8 是两者之间的差距清单。`bounded-list/` 已落地实现的**接口口径**以 [`BoundedList组件设计.md`](BoundedList组件设计.md) 为准（实现相对本文契约的偏离与新增见其 §15），测试与缺陷清单见 [`BoundedList测试方案.md`](BoundedList测试方案.md)。
