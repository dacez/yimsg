...
因此本节和 §8 的生产影响仍然成立；新核心自身在真实浏览器中确认的缺陷单独维护在 [`boundedlist/缺陷列表.md`](boundedlist/缺陷列表.md)。

### 3.1 独立核心与生产旧路径的职责边界

| 路径 | 文件 | 当前职责 | 状态 |
|---|---|---|---|
| 独立核心 | `bounded-list/bounded-list.ts` | 编排 PageWindow、StreamWindow、查询、轻通知、提示条、选择、生命周期与回调 | 已实现，尚有 OPEN 缺陷 |
| 独立核心 | `bounded-list/page-window.ts`、`page-source.ts` | 只读边界状态、server / local source、分页、裁剪、去重和本地变更 | 已实现，尚有容量 / 竞态缺陷 |
| 独立核心 | `bounded-list/stream-window.ts`、`update-pill.ts` | DOM 渲染、锚点、事件委托、键盘、提示条与 dispose | 已实现，尚有交互 / 生命周期缺陷 |
...
以下问题来自生产旧路径，按严重度排列。独立核心已经为其中多项提供统一能力，但在生产调用方完成迁移前，这些用户可见影响不会自动消失；独立核心自身的 11 个真实浏览器已知缺陷见 [`boundedlist/缺陷列表.md`](boundedlist/缺陷列表.md)。
...
| 范围 | 代码 | 测试 | 是否被生产视图使用 |
|---|---|---|---|
| 独立 BoundedList 核心 | `packages/uikit/src/app/bounded-list/` 九个文件 | `packages/uikit/tests/unit/bounded-list/`；`apps/web/tests/ui/bounded-list/` | 否 |
...
| 真实 Chromium 功能 / 性能专项 | 已实现；40 个用例中有 11 个唯一已知缺陷 |
| 页面级列表迁移 | 未开始 |
| 弹窗 / 面板列表迁移 | 未开始 |
| 六处未接入列表补齐 | 未开始 |
| 旧窗口和手写胶水删除 | 未开始 |
...
| 17 | 独立核心已有组件级与真实浏览器测试，但仍有 11 个 OPEN 缺陷，生产视图也尚未迁移 | 不能把测试已存在等同于组件已达到生产验收标准 | P1 |
...
**阶段 1：抽出组件外壳（已完成独立实现）**
`bounded-list/` 已实现 `createBoundedList`、`serverPageSource`、`localPageSource`、分页编排、`invalidate` 决策树、提示条、选择、`dispose`、事件回调、注册表与组件自身单测；尚未迁移调用方。进入阶段 2 前必须优先处理 [`boundedlist/缺陷列表.md`](boundedlist/缺陷列表.md) 中破坏数据正确性、交互和硬容量上限的 OPEN 缺陷。
...
| 层级 | 位置 | 当前口径 |
|---|---|---|
| Vitest 组件级 | `packages/uikit/tests/unit/bounded-list/*.test.ts` | 组件外壳、PageWindow、server / local PageSource、SelectionStore、registry、StreamWindow、提示条 |
| Chromium 功能 | `apps/web/tests/ui/bounded-list/bounded-list.spec.ts` | 34 个真实浏览器用例，覆盖全部参数、命令、回调、DOM 事件、并发、错误与生命周期 |
| Chromium 性能 | `apps/web/tests/ui/bounded-list/bounded-list.performance.spec.ts` | 6 个用例，覆盖 100,000 条本地数据、逻辑 1,000,000 条、250 页、10,000 次 invalidate、300 次创建 / 销毁和 1,000 次实时插入 |

截至 2026-07-28，功能 spec 为 23 项正常通过、11 项 `test.fail` 已知失败；性能 spec 为 5 项正常通过、1 项复用 `BL-BUG-010` 做容量已知失败。完整 40 项矩阵、阈值、单 worker / project dependency 和 `test.fail` 政策统一见 [`docs/development/测试方案.md` §7](../../../docs/development/测试方案.md#7-boundedlist-playwright-与性能专项)，11 个唯一缺陷见 [`boundedlist/缺陷列表.md`](boundedlist/缺陷列表.md)。

验收完成必须同时满足：

1. 所有 OPEN 缺陷对应的正确期望断言移除 `test.fail` 后通过，不得删除或放宽断言。
2. DOM 和数据窗口在 server page、local page 与连续 `upsertLocal` 三条路径都满足 `pageSize×maxPages` 硬上限。
3. 独立专项通过后，从仓库根目录执行 `./tools/run_all_tests.sh` 全量通过。
...
| [`BoundedList 缺陷列表`](boundedlist/缺陷列表.md) | 真实 Chromium 已知失败、复现证据、影响、定位与临时绕过 |
| [`测试方案.md` §7](../../../docs/development/测试方案.md#7-boundedlist-playwright-与性能专项) | 40 个 Playwright 功能 / 性能用例矩阵、数据级别、阈值和执行入口 |
