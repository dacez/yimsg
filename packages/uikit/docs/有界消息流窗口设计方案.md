...
> 入口关系：上级索引见 [`README.md`](../README.md)；本文是 UIKit 列表渲染的单一事实源，`UI设计方案.md` §8 保留概述性表格，细节以本文为准。§1–§6 是组件契约（目标态），§7 是当前代码逐文件对照（现状），§8 是两者之间的差距清单。
...
十二处。它们的数据类型不同（`LocalConversation` / `Message` / `Contact` / `GroupMember` / `TagChild`），交互不同（点开 / 多选 / 单选 / 跳转），刷新策略不同（实时重排 / 弹窗生命周期内静止），但**滚动、翻页、裁剪、锚点、空态、边界提示、加载态、身份去重、提示条、选中态**这十件事完全一样。任何一处单独实现这十件事，都会在某个边界组合上出错——事实上现在就出错了（§3.4）。
...
- **数据源 `PageSource`** 抽象「怎么取一页」。两种实现：
  ...
  - `localPageSource`：对一个已经全量加载到内存的数组做本地切片，游标就是下标。用于服务端没有对应查询能力、必须本地过滤排序的场景（提及群成员按拼音排序 + 子串过滤，§6.4）。本地游标不会外传给服务端，不违反「游标对客户端不透明」。
- **数据窗口**解决「数据太多」：内存里只保留完整数据集中用户附近的若干页，靠分页拉取扩展、超限时整页裁剪收缩（§2.3）；
- **渲染窗口**解决「DOM 太多」：因为数据窗口本身有上限，渲染窗口干脆等于数据窗口，**全部渲染**（§2.4）。

关键在于：把 `localPageSource` 也纳进同一个数据窗口，本地全量数据同样只有一个有界切片进 DOM。「一次性拉全量」和「一次性渲染全量」是两件事，前者在低频小集合上可以接受，后者永远不可以。
...
这五处是纯手写渲染，没有窗口、没有锚点、没有边界提示、没有分页：

| 场景 | 文件 | 现状 | 风险 |
|---|---|---|---|
| **提及群成员（@）** | `group-member-picker.ts` | 打开时按 `groupMemberPicker.maxPages`（200 页 × 40 = 8000 人）循环拉全量，批量取展示名，本地拼音排序，`for` 循环把**过滤后的全部条目**逐个 `appendChild` | 大群直接渲染数千个 DOM 节点；无键盘导航；无锚点 |
...
四个 SDK 方法（`searchMessages` / `searchContacts` / `getTags` / `getGroupMembers`）**全部支持 keyset 游标分页**，服务端能力是齐的，缺的只是客户端接入。唯一的例外是群成员没有服务端关键字搜索能力（`getGroupMembers` 只有 `cursor` / `backward` / `limit`），这正是 §2.2 引入 `localPageSource` 的原因。
...
组件化之后，十二处列表全部落在同一张表里。`freshEdge`、`selection`、`fetchByIdentity` 三列决定了它们行为上的全部差异。
...
| 提及群成员 | **`localPageSource`**（`getGroupMembers` 全量 + 拼音排序 + 子串过滤） | `userId` | head | single | — | 无 | **未接入** |
...
用户点名的第二个场景，也是当前**最需要改造**的一个：输入 `@` 弹出成员选择器。

现状（`group-member-picker.ts`）：循环拉最多 200 页（8000 人）→ 批量取展示名 → 按拼音排序 → 关键字子串过滤 → **把过滤后的全部条目逐个 `appendChild`**。八千人的群就是八千个 DOM 节点，正是有界列表要消灭的东西。

难点在于：`getGroupMembers` **没有服务端关键字搜索能力**（只有 `cursor` / `backward` / `limit`），而按拼音排序也只能在客户端做（服务端排序是 role 倒序 + uid 升序）。所以「服务端分页 + 服务端过滤」这条路走不通。

`localPageSource` 就是为此设计的：
...
| 5 | 提及群成员未接入组件 | 大群渲染数千 DOM 节点；无键盘导航；无锚点 | P1 |
| 6 | 身份键双口径（`identityOf` / `keyOf`），靠注释对齐 | 任一处改动不同步就会重复渲染或锚点失效 | P1 |
| 7 | 缺 `PageSource` 抽象（尤其 `localPageSource`） | 需要本地过滤 / 排序的场景无法复用有界窗口 | P1 |
...
| 15 | 四处搜索 / 组织架构列表无分页，硬截断（8 / 20 / 30 / 160 / 200 条） | 结果被静默丢弃，用户无从知晓 | P2 |
...
**阶段 4：补齐未接入的列表**
实现 `localPageSource` 与键盘导航，改造提及群成员选择器；再把全局搜索、会话内搜索、我发出的请求、组织架构浏览四处接入（都是 `serverPageSource` + 几行配置）。
