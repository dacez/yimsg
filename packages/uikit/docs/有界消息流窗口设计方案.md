...
| Chromium 功能 | `apps/web/tests/ui/bounded-list/bounded-list.spec.ts` | 50 个真实浏览器用例，覆盖参数、命令、回调、DOM 事件、并发、错误与生命周期 |
...
4. 从仓库根目录执行 `./tools/run_all_tests.sh` 全量通过；其 UI 步骤以 `--project=chromium --retries=0` 包含 BoundedList 功能 spec，但按要求不运行性能 spec。
