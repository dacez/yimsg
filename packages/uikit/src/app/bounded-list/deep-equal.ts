// 结构相等比较：只看结构不看引用。组件里两处共用同一份语义——
// 渲染引擎判断「这一行的数据变了没有，能不能复用已有 DOM」，组件外壳判断「当前查询
// 条件是不是仍等于初始查询条件」（据此决定空态显示「暂无数据」还是「无搜索结果」）。
//
// 语义要点：环引用用 WeakMap 记账，Date / TypedArray 按值比较，原型不同直接判不等
// （不把 class 实例当成普通对象逐键比）。**不能退化成 JSON.stringify**：它对
// `{a:1,b:2}` 与 `{b:2,a:1}` 给出不同结果，键序不同就会把查询误判成「已过滤」
// （`BL-UNIT-BUG-012`）。
